package test

import (
	"encoding/json"
	"fmt"
	"geometris-go/configuration"
	"geometris-go/convert"
	"geometris-go/core/device"
	"geometris-go/core/sensors"
	"geometris-go/logger"
	"geometris-go/message/factory"
	"geometris-go/message/types"
	"geometris-go/parser"
	pTypes "geometris-go/types"
	"reflect"
	"testing"
)

func TestConvertor(t *testing.T) {
	messageFactory := factory.New()
	rawMessage := messageFactory.BuildMessage([]byte{}).(*types.RawLocationMessage)
	messageParser := parser.NewWithDiffConfig("..", "/config/initializer/ReportConfiguration.xml")
	syncParam := parser.NewSynchParameter("28.65.60.9.36.3.4.7.8.11.12.14.17.24.50.56.51.55.70.71.72.73.74.75.76.77.80.81.82")
	parsedMessage := messageParser.Parse(rawMessage, syncParam)
	state := device.NewSensorBasedState([]sensors.ISensor{})
	conv := convert.NewStateToDTO(device.NewStateBasedState(state, parsedMessage.(*types.LocationMessage).Sensors()).State())
	dtoMessage := conv.Convert()
	strMess, jerr := json.Marshal(dtoMessage)
	if jerr != nil {
		t.Error("Error while marshaling")
	}
	should := "{\"sid\":0,\"Data\":{\"Ack\":1617111127,\"CoolantTemp\":0,\"CumulativeFuelEconomy\":0,\"DTCCode\":[\"P0016\",\"P1190\",\"P1188\"],\"Duration\":357,\"EngineOdometer\":0,\"EngineSpeed\":0,\"FenceID\":0,\"FuelLevel\":0,\"GpsValidity\":0,\"Heading\":0,\"IgnitionDuration\":0,\"IgnitionState\":0,\"Latitude\":0,\"LocationAge\":999,\"Longitude\":0,\"OdometerMil\":0,\"PacketContent\":\"9386\",\"RPM\":0,\"ReportClass\":\"OFF_PERIODIC\",\"Satellites\":0,\"SequenceID\":351,\"SpeedMPH\":0,\"ThrottlePosition\":0,\"TimeStamp\":\"2021-03-30T13:32:07Z\",\"TotalIdleDuration\":0,\"TripFuelEconomy\":0,\"VIN\":\"\"}}"
	logger.Logger().WriteToLog(logger.Info, string(strMess))
	if should != string(strMess) {
		t.Error("Unexpected DTO")
	}
}

func TestDecoder(t *testing.T) {
	dtoMessage := "{\"sid\":0,\"Data\":{\"Ack\":1617111127,\"CoolantTemp\":0,\"CumulativeFuelEconomy\":0,\"DTCCode\":[\"P0016\",\"P1190\",\"P1188\"],\"Duration\":357,\"EngineOdometer\":0,\"EngineSpeed\":0,\"FenceID\":0,\"FuelLevel\":0,\"GpsValidity\":0,\"Heading\":0,\"IgnitionDuration\":0,\"IgnitionState\":0,\"Latitude\":0,\"LocationAge\":999,\"Longitude\":0,\"OdometerMil\":0,\"PacketContent\":\"9386\",\"RPM\":0,\"ReportClass\":\"OFF_PERIODIC\",\"Satellites\":0,\"SequenceID\":351,\"SpeedMPH\":0,\"ThrottlePosition\":0,\"TimeStamp\":\"2021-03-30T13:32:07Z\",\"TotalIdleDuration\":0,\"TripFuelEconomy\":0,\"VIN\":\"\"}}"
	messageFactory := factory.New()
	rawMessage := messageFactory.BuildMessage([]byte{}).(*types.RawLocationMessage)
	messageParser := parser.NewWithDiffConfig("..", "/config/initializer/ReportConfiguration.xml")
	syncParam := parser.NewSynchParameter("28.65.60.9.36.3.4.7.8.11.12.14.17.24.50.56.51.55.70.71.72.73.74.75.76.77.80.81.82")
	parsedMessage := messageParser.Parse(rawMessage, syncParam)
	config := configuration.NewReportConfig(pTypes.NewFileWithDir("..", "/config/initializer/ReportConfiguration.xml"))
	conv := convert.NewDTOToState(dtoMessage, config)
	convSensors := conv.Convert()
	sensorsMap := make(map[string]sensors.ISensor)
	for _, s := range convSensors {
		sensorsMap[s.Symbol()] = s
	}
	locationMessageSensors := parsedMessage.(*types.LocationMessage).Sensors()
	for _, s := range locationMessageSensors {
		if s.Symbol() == "Ack" {
			continue
		}
		if value, f := sensorsMap[s.Symbol()]; f {
			if !reflect.DeepEqual(value, s) {
				t.Error(fmt.Sprintf("Unexpected sensors value dto: %v; parsed: %v", value, s))
			}
		} else {
			t.Error(fmt.Sprintf("Cant find sensor by key: %v", s.Symbol()))
		}
	}
}
